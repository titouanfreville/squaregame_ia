...
let rec processPlay grid i j side score f =
  if allClosed grid 
  then (grid, score)
  else
    let (g, ok, replay) = addPlay grid i j side in
    if ok 
    then
      begin
        if replay
...
(* Ia calls *)
let playNoob grid score = (grid, score)

let rec playIa0 grid score = 
  let ((i,j), s) = Base.randomMove (play0 grid []) in
  processPlay grid i j s score playIa0
...
let rec move grid score =
  ...
  processPlay grid i j side score move
